...
function initSelectList() {
	callAjax('role/list', {}, function(data) {
		printSelect("roleId",data.dataList,"id","name");
	});
}
...
function printTable(data) {
	...
	for (var x = 0; x < data.length; x++) {
	    html += '<tr>';
	    html += 	'<td>' + (x+1) + '</td	>';
	    html += 	'<td>' + isnull(data[x].login_name) + '</td>';
	    html += 	'<td>' + isnull(data[x].name) + '</td>';
	    html += 	'<td>' + isnull(data[x].roleName) + '</td>';
	    if(data[x].status == 0){
	    	html += 	'<td>启用</td>';
...
function  openThis(id){
	callAjax("common/change", {tableName:"user",value:"0",keyValue:id}, function(data) {
...
function  closeThis(id){
	callAjax("common/change", {tableName:"user",value:"1",keyValue:id,}, function(data) {
		if(!(data.code == 200 && data.data)) {
			layer.msg("停用失败");
			return;
		}
...
function deleteThis(id) {
	layer.open({
		  ...
		  ,yes: function(index, layero){
			...
			callAjax("common/change", {tableName:"user",value:"9",keyValue:id,}, function(data) {
				layer.msg("删除成功");
				initTable();
			});
		  }
...
function  modify(id){
	...
	if(id){
		callAjax("user/byid", {id:id}, function(data) {
			doModify(data.data);
		});		
	}
